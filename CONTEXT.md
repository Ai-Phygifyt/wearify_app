...
### Customer PWA onboarding redesign — new launch + auth flow from reference designs

- **Why:** the user supplied pixel reference designs for the customer-facing `/c` module and asked for an exact, mobile-responsive rebuild of the entry experience. Approach agreed up front: **keep all Convex wiring/routing, restyle UI only.** Assets live under `public/customer/{first-screen,second-screen,third-screen,login}/` (SVGs the user provided — some are large embedded-raster SVGs, referenced by URL, never inlined).
- **New end-to-end flow:**
  - `splash 1` (saree-illustration backdrop + frosted logo badge) → `splash 2` (white + coral-pink corner glows + Wearify wordmark) → `/c/welcome` (for logged-out users) → `/c/login` **or** `/c/register` → `/c` home.
  - Authenticated users still skip straight to home after the two splash stages. Session-expiry still routes to `/c/login`.
- **Two-stage splash** ([app/c/layout.tsx](app/c/layout.tsx)): replaced the single 1.2s `Splash` placeholder (a CSS "W") with a real two-stage sequence driven by `splashStage` state (0→1→2) and two timers (`SPLASH_ONE_MS=1300`, `SPLASH_TWO_MS=1600`). `Splash` = full-bleed `first-screen/background.svg` over `#FBF7F1` + centered `first-screen/logo.svg`. `SplashTwo` = white field with coral-pink radial glows + centered `second-screen/logo.svg`. Both show a bottom "©copyright wearify techno services pvt.ltd." line.
  - **Frosted-disc gotcha (worth remembering):** `first-screen/logo.svg` uses an internal `backdrop-filter` for its glass look, but **`backdrop-filter` inside an SVG loaded via `<img>` does NOT blur the page behind it** — only within the SVG. The SVG's own circle is `white @ 45%`, so the busy saree pattern bled through and looked messy. Fix: render a real `backdrop-filter: blur(12px)` translucent-white disc (≈96% of the logo width, centered) behind the `<img>` in `Splash`. Don't "simplify" that disc away.
- **`/c/welcome` (NEW route)** ([app/c/welcome/page.tsx](app/c/welcome/page.tsx)): brand entry screen. White + top-right/bottom-left pink glows, "Wearify" wordmark + "Eng ⌄" pill top bar, `third-screen/image.svg` phone-mockup hero, "Find Your Perfect Look" heading, italic maroon subtitle, the user-provided `third-screen/button.svg` used directly as the "Let's Get Started →" CTA (→ `/c/register`), and "Already have an account? Login" (→ `/c/login`). Added `/c/welcome` to `PUBLIC_ROUTES`, and the layout's **no-token redirect changed from `/c/login` → `/c/welcome`**.
- **Login restyle** ([app/c/login/page.tsx](app/c/login/page.tsx)): dropped the dark `--cx-grad-hero` theme + floating `Orb`s for a light theme matching the set — white + pink corner glows, `login/logo.svg` + "Your AI - Powered Saree Experience" tagline (the tagline is **separate text**, not in the SVG), white card. Phone step: "Welcome Back" + `+91` box + input + maroon "Send OTP →" pill + "New user? Register". OTP step: left-aligned "Verify OTP", "ENTER OTP" label, 6 grid boxes (maroon border when filled), "Verify OTP →", "Resend OTP". **Back arrow removed from the OTP step to match the reference** (user can re-enter via Resend; flag if a change-number affordance is wanted). All OTP/Convex logic (`loginWithOtp`, paste/auto-advance, no-account panel) preserved verbatim.
- **Register simplified + restyled** ([app/c/register/page.tsx](app/c/register/page.tsx)): full rewrite to a light 3-step (`phone → otp → details`) matching the reference. **The "details" step now collects ONLY Full Name + Date of Birth** (reference showed nothing else). DOB uses a styled `DD - MM - YYYY` display with a `Calendar` icon and a transparent native `<input type="date">` overlaid on top (most reliable cross-mobile picker trigger — chosen over `showPicker()`, which is flaky on older iOS Safari).
  - **Dropped fields:** gender, height, city, email, profile photo (and all the height-unit/cm↔ft-in helpers, photo upload via `useUploadFile`/`GUARDS.customerPhoto`). `completeProfile` (server) still **requires** `name`, valid `dateOfBirth`, and `heightCm ∈ [50,250]`; `gender`/`city` are accepted blank. So sign-up passes `gender:""`, `city:""`, and **`heightCm: 160` (`DEFAULT_HEIGHT_CM`)**. Rationale: the kiosk body-scan captures real sizing for try-on; the dropped fields are editable later in the customer profile. **If accurate height/gender at sign-up matters, this is the line to revisit** (`convex/customers.ts` `completeProfile` is unchanged — the simplification is purely client-side).
- **Design system note:** all five screens share `MAROON = "#6E262B"` (close to but not identical to the `button.svg` maroon `#68262A`), the coral-pink glow gradient string (top-right + bottom-left, "darker + more spread" per user feedback), and DM Sans (the module's loaded sans — Cormorant/DM Mono are the other two; headings here are sans, not the usual italic serif). These are inline per-file constants, not yet centralized in `customer-theme.css`.
- **Files touched:** `app/c/layout.tsx` (two-stage splash + welcome route + redirect), `app/c/welcome/page.tsx` (new), `app/c/login/page.tsx` (light restyle, OTP refine), `app/c/register/page.tsx` (rewrite + simplify). New assets under `public/customer/`.
- **Verification:** `tsc --noEmit` clean on all changed files; ESLint clean except the **pre-existing** `set-state-in-effect` on login's `useEffect(()=>setMounted(true),[])` (unchanged, just moved up after Orb removal); all routes serve 200 in dev. Live screenshot verification was NOT done — gstack `/browse` skill is not installed in this environment.
- **Open / flagged for the user:** (a) "Eng ⌄" pill on welcome is styled but inert (no pre-login language picker yet); (b) reference text says "Resent OTP" — shipped as the correct "Resend OTP"; (c) register no-account path still re-asks for OTP (login verifies OTP, routes to `/c/register?phone=X`, which verifies again — pre-existing redundancy, left as-is); (d) not yet committed (user reviewing screen-by-screen); (e) post-login app screens still to come.
...
- **URLs:**
  ...
  - Dev Convex (still active): `dev:formal-snake-780` — keep using this for local work; don't cross-wire. **This deployment holds the seeded dev data and is now pinned via `CONVEX_DEPLOY_KEY` in `.env.local` — see "Local dev Convex deployment" below for why.**
...
### Local dev Convex deployment — why `.env.local` is pinned to formal-snake-780 (2026-06-06 incident)

- **TL;DR:** the seeded dev data lives in **`dev:formal-snake-780`** (reference `dev/ai-development`, team `ai-development`, project `wearify`, region eu-west-1). `.env.local` (gitignored) now pins it via `CONVEX_DEPLOY_KEY`. If local dev ever shows an empty database again, this section is why.
- **Per-developer dev deployments are the trap.** Convex provisions a *separate* cloud dev deployment for each logged-in developer per project. The data was seeded under developer `ai-development` (→ `formal-snake-780`). The Convex CLI on this machine is logged in as **`abhigna-d-a`**, whose wearify dev deployment is a *different, empty* one: **`curious-raven-304`** (reference `dev/abhigna-d-a`). The `wearify` project record's `devDeploymentName` is `curious-raven-304`, prod is `quirky-narwhal-971`.
- **What happened.** Running `npm run dev` with no valid `CONVEX_DEPLOYMENT` in `.env.local` caused the `predev`/`dev:backend` scripts (`convex dev`) to resolve to the logged-in developer's own deployment, provision/select the empty `curious-raven-304`, and **rewrite `.env.local`** — so the app showed an empty DB. No data was lost; it was always intact in `formal-snake-780` (and prod `quirky-narwhal-971` was never touched).
- **Why hand-editing `.env.local` didn't work.** `convex dev` re-resolves the deployment from the login identity on every run and overwrites `CONVEX_DEPLOYMENT` / `NEXT_PUBLIC_CONVEX_URL`. `npx convex deployment select dev/ai-development` also got reverted on the next `convex dev`.
- **The fix (applied).** Added `CONVEX_DEPLOY_KEY=dev:formal-snake-780|...` to `.env.local`. A deploy key forces every `convex dev` / `npm run dev` to target `formal-snake-780` regardless of which developer is logged in, and it stops rewriting the file. The key reuses the deployment's existing "netlify" deploy key.
- **If it breaks again:** confirm `.env.local` still has the `CONVEX_DEPLOY_KEY` line. If that key was rotated, mint a fresh **dev** deploy key for `formal-snake-780` in the Convex dashboard (Deployment → Settings → Deploy Keys) and replace the value. Alternatively, migrate the data from `formal-snake-780` → your own `curious-raven-304` (`npx convex export` / `import`) so the standard per-developer flow works without the key.
- **Also surfaced that day:** the team is over the Convex **free-plan limit** (dashboard banner) — trim usage or upgrade before it interrupts service. Separately, the Vercel deploy of `main` had failed earlier on `ERR_PNPM_OUTDATED_LOCKFILE` because `package.json` gained `playwright-core` without a lockfile bump; fixed by regenerating `pnpm-lock.yaml` (sync the lockfile in the same commit whenever deps change).
