...
> **OTP path note:** OTP is now a real MSG91 send (see §12 "Real MSG91 OTP" entry). The seed phones below are fake (9800000001 etc.) so they will NOT receive SMS — for those accounts, log in with the **password** column. Use real phones if you specifically want to exercise the OTP path end-to-end.

**Stores** — password `Store@123`:
...
**Tailors** — password `Tailor@123`: Manoj Darji (TL-001, Mumbai), Geeta Bai (TL-004, Chennai)

**Kiosk pairing:** new flow — admin or store-owner generates a 6-digit code at `/admin/devices` (Pairing tab) or `/store/settings/kiosks`, kiosk technician types it at `/kiosk/setup`. The legacy "Tablet Code 123456" is gone. Customer-side trial-room codes are still per-session, generated by tablet.
...
### Real MSG91 OTP end-to-end — replacing the `123456` stub

- **Why:** `phoneAuth.verifyOtp` and `phoneAuth.loginWithOtp` were both checking `args.otp !== "123456"` and returning success otherwise. Hardcoded test creds in production. Pattern borrowed from `/home/vrathik/Documents/comfyui_next` (its `convex/otp.ts` and `app/api/otp/*` routes) — only the OTP plumbing was lifted, nothing else.
- **Schema** ([convex/schema.ts](convex/schema.ts)): new `otpSessions` table — `{ phone, requestId, verified, attempts, expiresAt, createdAt }` indexed `by_phone`. The OTP itself lives in MSG91; we only track the Convex side so downstream mutations can verify the phone was confirmed and rate-limit verify attempts without hitting MSG91 every time.
- **Backend module** ([convex/otp.ts](convex/otp.ts)):
  - `createOtpSession({ phone, requestId })` — wipes any prior row for that phone (so a stale `verified: true` from a previous flow can't be replayed) and inserts a fresh one.
  - `markOtpVerified({ phone })` — flips `verified: true` on the row. Throws if missing/expired (10-min TTL).
  - `incrementAttempt({ phone })` — bumps attempts, throws at >5. Called BEFORE the MSG91 verify call so a burst of bad guesses can't bypass the cap by skipping the round trip.
  - `isPhoneVerified(ctx, phone)` and `consumeVerifiedSession(ctx, phone)` — internal helpers used by `phoneAuth`. `consume` deletes the row on successful login so a single verified session can't be replayed against another role or re-mint a session on another device.
  - `checkStatus({ phone })` — public query for any UI that wants to render "OTP sent, X attempts left".
- **`phoneAuth` rewire** ([convex/phoneAuth.ts](convex/phoneAuth.ts)): both `verifyOtp` and `loginWithOtp` now call `isPhoneVerified` instead of comparing to `"123456"`. The `otp` arg is kept for backwards compatibility but ignored — MSG91 already validated it. Each successful return path in `loginWithOtp` (store_owner / customer / tailor) calls `consumeVerifiedSession(ctx, phone)` before returning.
- **MSG91 routes** (Next.js — Netlify runtime, NOT Convex):
  - [app/api/otp/send/route.ts](app/api/otp/send/route.ts): POST `https://control.msg91.com/api/v5/otp` then writes the Convex session inline (one round trip from the client). Passes `otp_length: 6` in the body — the template's default is 4, but our UI has 6 boxes with auto-submit, so these MUST agree (fixed in a follow-up after the user reported 4-digit SMS).
  - [app/api/otp/verify/route.ts](app/api/otp/verify/route.ts): bumps Convex attempt counter first, then MSG91 verify, then `markOtpVerified`. The order matters — flipping it would let a rogue client spam MSG91 by skipping the rate-limit mutation.
  - [app/api/otp/resend/route.ts](app/api/otp/resend/route.ts): POST MSG91 retry; doesn't touch Convex (the OTP value is unchanged, the existing session is still valid).
  - All three short-circuit with a 500 + clear error if `MSG91_AUTH_KEY`/`MSG91_TEMPLATE_ID` are unset, instead of falling through to a fetch with `?authkey=undefined`.
- **Convex client for API routes** ([lib/convex-server.ts](lib/convex-server.ts)): lazy `ConvexHttpClient` reading `NEXT_PUBLIC_CONVEX_URL`. Separate from `lib/auth-server.ts` (which is for Better Auth-aware calls forwarding the user session token). Use this one for server-side calls that don't need the user session — e.g. `/api/otp/*`.
- **Client helper** ([lib/otp.ts](lib/otp.ts)): `sendOtp(phone)` / `verifyOtpCode(phone, otp)` / `resendOtp(phone)` returning a uniform `{ success, error? }` shape so every UI surface shares error rendering.
- **Surfaces wired (7):** `/c/login`, `/c/register`, `/store/login`, `/tailor/login`, `/tablet/phone`, `/tablet/register`, and the kiosk's `PhoneAuthScreen` + `OTPScreen` (inside [app/kiosk/page.tsx](app/kiosk/page.tsx)). Each: send on phone-submit / Send-OTP click, verify before final login mutation, resend on the Resend button. The kiosk's pre-existing top-level `verifyOtpMut = useMutation(api.phoneAuth.verifyOtp)` was unused dead code and got removed in the same pass.
- **Improvements over the reference impl:**
  - Reference made the client do `MSG91 → API route → Convex` in two round trips; a dead browser between them could leave MSG91 having sent an SMS with no Convex trace. Ours fuses the Convex write into the API route handler.
  - Reference incremented attempts AFTER the MSG91 round trip; ours bumps BEFORE so spamming bad codes burns through the budget cleanly.
  - Reference left `verified: true` rows around indefinitely; we consume on use (replay protection).
- **Env vars** — Netlify-only (Next.js runtime reads them in `/api/otp/*`). Convex needs nothing new because OTP doesn't run in the Convex runtime here. Names: `MSG91_AUTH_KEY`, `MSG91_TEMPLATE_ID`. Placeholders are in [.env.local](.env.local) (dev) and [.env.prod](.env.prod) (memo for Netlify, gitignored).
- **MSG91 quirks worth knowing:**
  - Template length default may be 4 digits — pass `otp_length` per request to override. We hardcode `6`.
  - Send returns `type: "error"` with `"already"` in the message when an active OTP still exists for that phone — we treat that as success since the existing code is still valid.
  - Phones must be in canonical Indian form: 10 digits → prepend `91`, 11–12 digits used as-is, no `+` prefix on the wire.
- **Deferred (flagged):**
  - `customers.ensureByPhone` (the kiosk new-customer creation path) is still unauthenticated. Not a hole because it's called immediately after a verified OTP in normal flow, but a malicious client could spam dummy customers by skipping the OTP step. Tighten by gating on `isPhoneVerified` if/when this becomes an issue.
  - Tablet-side staff PIN flow doesn't touch OTP at all — orthogonal.
  - Password-based login (`loginWithPassword`) does not require an OTP. By design — it's the fallback for users who set a password — but worth noting if you ever want OTP-required-for-password-reset.
